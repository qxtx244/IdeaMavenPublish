package com.qxtx.idea.gradle.mavenpublish_java.impl

import com.qxtx.idea.gradle.mavenpublish_java.Base
...
class MavenPublishImpl extends Base {
            ...
            publicationContainer.create(target.name, MavenPublication.class) { publication ->
                ...
                if (pubSourceEnable && pubSrcDirs != null && !pubSrcDirs.isEmpty()) {
                    ...
                    if (sourcesJarTask == null) {
                        try {
                            sourcesJarTask = target.task([type: org.gradle.jvm.tasks.Jar, group: EXT_TASK_GROUP_NAME], sourcesJarTaskName) {
                ...
                if (pubJavaDocEnable) {
                    ...
                    try {
                        ...
                        if (javadocJarTask == null) {
                            javadocJarTask = target.task([type: org.gradle.jvm.tasks.Jar, group: EXT_TASK_GROUP_NAME, dependsOn: javadocTask], javadocJarTaskName) {
                ...
                if (pubGroovyDocEnable) {
                    //打包 groovydoc
                    def groovydocTask = target.tasks.findByName('groovydoc')
                    if (groovydocTask != null) {
                        def groovydocJarTask = target.task([type: org.gradle.api.tasks.bundling.Jar, group: EXT_TASK_GROUP_NAME, dependsOn: groovydocTask], 'jarGroovydoc') {
    ...
    private String getPubArchivePath(Project target) {
        ...
        if (path == null) path = "${target.buildDir.absolutePath}/libs/${target.name}.jar"
