package com.qxtx.idea.gradle.mavenpublish_android_dokka.impl

import com.qxtx.idea.gradle.mavenpublish_android_dokka.Base
...
class MavenPublishImpl extends Base {
    ...
    void start(Project target) {
        ...
        def pubHtmlDocEnable = getPubHtmlDocEnable()
        ...
        println("${target.name}发布配置：" +
                ...
                "\nenable htmlDoc=$pubHtmlDocEnable")
        ...
        if (dokkaPlugin != null) {
            ...
            if (dokkaJavadocTask != null) dokkaJavadocTask.each { doOffline(it) }
            def dokkaHtmldocTask = target.tasks.findByName('dokkaHtml')
            if (dokkaHtmldocTask != null) dokkaHtmldocTask.each { doOffline(it) }
        }
        ...
        publishing.publications {publicationContainer ->
            ...
            publicationContainer.create(target.name, MavenPublication.class) { publication ->
                ...
                //打包 htmldoc
                if (pubHtmlDocEnable && dokkaPlugin != null) {
                    try {
                        def htmlDocJarTaskName = "jarHtmlDoc"
                        def htmlDocJarTask = target.tasks.findByName(htmlDocJarTaskName)
                        if (htmlDocJarTask == null) {
                            def dokkaHtmlTask = target.tasks.findByName('dokkaHtml')
                            if (dokkaHtmlTask == null) throw new IllegalArgumentException("dokkaHtml task is not exist!")

                            htmlDocJarTask = target.task([type: org.gradle.jvm.tasks.Jar, group: EXT_TASK_GROUP_NAME, dependsOn: dokkaHtmlTask], htmlDocJarTaskName) {
                                getArchiveClassifier().set('htmldoc')
                                def dirList = new ArrayList()
                                dokkaHtmlTask.each { dokkaTask ->
                                    //让 dokka 离线工作，防止一直下不了东西导致耗时超长
                                    dokkaTask.offlineMode = true
                                    dokkaTask.dokkaSourceSets {
                                        named('main') {t ->
                                            t.noAndroidSdkLink = true
                                ...
                                doLast {
                                    println "dokkaHtml Jar 输出目录：${Arrays.toString(dirList.toArray(new String[0]))}"
                                }
                            }
                        }
                        artifact(htmlDocJarTask.archiveFile) {
                            it.builtBy htmlDocJarTask
    ...
    /**
     * 获取是否发布 htmlDoc。如果未配置，默认返回 false
     * @return true 表示发布，否则不发布
     */
    private boolean getPubHtmlDocEnable() {
        config.pubHtmlDocEnable
    }
